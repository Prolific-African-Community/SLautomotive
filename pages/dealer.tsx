...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen font-sans bg-black text-white">
      ...
      <aside className="w-64 bg-black text-white flex flex-col px-4 space-y-6 shadow-xl">
        <a href="/">
          <img src="/logo-sl-automotive.jpg" alt="SL Automotive" className="h-56 w-56 flex mx-0" />
        </a>
      ...
      <main className="flex-1 overflow-y-auto pr-48 py-16 relative">
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 max-w-7xl mx-auto">
            {vehicles.map((v) => (
              <div
                key={v.stockId}
                className="bg-[#111113] rounded-2xl shadow-md overflow-hidden hover:shadow-lg hover:scale-[1.02] transition cursor-pointer border border-white/5"
        ...
        <section className="mb-24">
          ...
          <div className="flex justify-center flex-wrap gap-8 max-w-5xl mx-auto">
            {[
              ...
              <div key={label} className="bg-[#111113] rounded-xl shadow p-6 w-64 text-center border border-white/5">
                <h3 className="text-4xl font-bold text-yellow-400">{value}</h3>
                <p className="text-sm text-white-300 mt-2">{label}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* MODAL DÉTAIL VÉHICULE — scrollable */}
        {selectedCar && (
  <div
  ...
  >
    {/* MODAL épuré — image au centre + specs autour (pas de scroll interne) */}
...
<div className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-6 px-6 md:px-8 py-6 items-start">
  ...
  <p className="text-xs uppercase tracking-widest text-zinc-400 mb-2">Description</p>
    {[
      {
        label: "Boîte",
        value: selectedCar.boite,
        icon: (
    ...
    {/* Options en chips sous les specs */}
